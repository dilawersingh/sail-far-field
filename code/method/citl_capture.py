...
Shared camera-capture helpers for every CITL-based script (SAIL, SAIL+,
B2/camera_feedback_gs_gd). Previously copy-pasted near-identically into all
three -- consolidated here so a fix (like this file's own calibration
method) only needs to happen once, not three times.

Calibration method: OLD (rotate full frame -> slice fixed ROI -> cv2.resize),
NOT the newer homography/warpPerspective approach. Reverted project-wide
after confirming, via matched-condition comparison, that the old method
outperforms homography under this rig's current physical setup -- see
project notes from 2026-07-16. This is a similarity transform (5 DOF:
translation, rotation, size) and genuinely cannot represent keystone/
trapezoidal warp the way the homography approach could; that capability is
being knowingly traded away based on empirical results, not an oversight.

Config shape this expects (matches what SAIL/SAIL+/B2 already read):
  hardware.manual_alignment: {x0, y0, w, h, rotation_deg}
  -> passed here as roi={"y0":.., "x0":.., "h":.., "w":..} and angle=rotation_deg
"""
...
# Normalization
# -----------------------------
def normalize_intensity_sum_excluding_mask(I, exclude_mask=None, eps=1e-12, scale_to_numel=True):
    if I.ndim < 2:
...
# -----------------------------
# Main capture loader -- OLD calibration method
# -----------------------------
def load_camera_capture_for_citl(
    ...
    """
    Load grayscale DSLR capture, rotate the full frame, crop a fixed ROI,
    resize to out_hw, and normalize while excluding the DC region from the
    ...
    I_cam = normalize_intensity_sum_excluding_mask(
        I_cam_raw, exclude_mask=dc_mask_t, eps=eps_norm, scale_to_numel=True
    )
